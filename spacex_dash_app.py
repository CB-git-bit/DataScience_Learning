...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        # Get total count of success launches for all sites
        success_count = spacex_df[spacex_df['class'] == 1]['class'].count()
        fig = px.pie(values=[success_count, len(spacex_df)-success_count], 
        ...
        # Get success and failed count for the selected site
...
def get_scatter_chart(entered_site,payload_range):
    if entered_site == 'ALL':
        ...
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', color='Booster Version Category',
                         title=f'Payload and Success rate for site {entered_site}')
        return fig
